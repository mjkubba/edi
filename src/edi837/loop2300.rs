...
pub fn parse_loop2300(content: &str) -> (Loop2300, String) {
    ...
    while let Some(pwk_pos) = remaining_content.find("PWK*") {
        ...
        let pwk_end = remaining_content[pwk_pos..]
            .find('~')
            .unwrap_or(remaining_content.len())
            + pwk_pos;
        loop2300
            .pwk
            .push(remaining_content[pwk_pos..=pwk_end].to_string());
    ...
    while let Some(nte_pos) = remaining_content.find("NTE*") {
        ...
        let nte_end = remaining_content[nte_pos..]
            .find('~')
            .unwrap_or(remaining_content.len())
            + nte_pos;
        loop2300
            .nte
            .push(remaining_content[nte_pos..=nte_end].to_string());
    ...
    while let Some(hi_pos) = remaining_content.find("HI*") {
        ...
        let hi_end = remaining_content[hi_pos..]
            .find('~')
            .unwrap_or(remaining_content.len())
            + hi_pos;
        loop2300
            .hi
            .push(remaining_content[hi_pos..=hi_end].to_string());
    ...
    while let Some(amt_pos) = remaining_content.find("AMT*") {
        ...
        let amt_end = remaining_content[amt_pos..]
            .find('~')
            .unwrap_or(remaining_content.len())
            + amt_pos;
        loop2300
            .amt
            .push(remaining_content[amt_pos..=amt_end].to_string());
    ...
    while let Some(qty_pos) = remaining_content.find("QTY*") {
        ...
        let qty_end = remaining_content[qty_pos..]
            .find('~')
            .unwrap_or(remaining_content.len())
            + qty_pos;
        loop2300
            .qty
            .push(remaining_content[qty_pos..=qty_end].to_string());
    ...
    if let Some(hcp_pos) = remaining_content.find("HCP*") {
        ...
        {
            let hcp_end = remaining_content[hcp_pos..]
                .find('~')
                .unwrap_or(remaining_content.len())
                + hcp_pos;
    ...
    while let Some(cl1_pos) = remaining_content.find("CL1*") {
        ...
        let cl1_end = remaining_content[cl1_pos..]
            .find('~')
            .unwrap_or(remaining_content.len())
            + cl1_pos;
        loop2300
            .cl1_segments
            .push(remaining_content[cl1_pos..=cl1_end].to_string());
