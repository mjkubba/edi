...
pub fn parse_loop2000c(content: &str) -> (Loop2000c, String) {
    ...
    // Parse DMG before NM1*QC (some files have DMG at Loop2000C level)
    ...
    // Parse NM1*QC (Patient Name - Loop2010CA)
    if let Some(nm1_pos) = remaining_content.find("NM1*QC*") {
            ...
            && !remaining_content[..nm1_pos].contains("CLM*")
        {
            ...
            // Parse DMG after NM1*QC/N3/N4 if not already found
            if loop2000c.dmg.is_none() {
                if let Some(dmg_pos) = remaining_content.find("DMG*") {
                    if !remaining_content[..dmg_pos].contains("HL*")
                        && !remaining_content[..dmg_pos].contains("NM1*")
                        && !remaining_content[..dmg_pos].contains("CLM*")
...
mod tests {
    ...
    fn test_parse_loop2000c() {
        let content = "HL*3*2*23*0~PAT*19~DMG*D8*19800519*M~NM1*QC*1*DOE*JANE****MI*123456789B~N3*123 MAIN ST~N4*ANYTOWN*PA*17111~REF*SY*987654321~DTP*304*D8*20230101~";

        let (loop2000c, _) = parse_loop2000c(content);

        assert_eq!(loop2000c.hl, "HL*3*2*23*0~");
        assert_eq!(loop2000c.pat, "PAT*19~");
        assert_eq!(loop2000c.dmg, Some("DMG*D8*19800519*M~".to_string()));
        assert_eq!(
            loop2000c.nm1_patient,
            Some("NM1*QC*1*DOE*JANE****MI*123456789B~".to_string())
        );
