...
pub fn get_loop_2110s(mut contents: String) -> (Vec<Loop2110>, String) {
    ...
    for _ in 0..ik4_count {
        ...
        if contents.contains("IK4") {
            ...
            let end_pos = if let Some(next_ik4_pos) = find_next_segment_start("IK4", &contents, 3) {
                // If there's another IK4 at a segment boundary, extract up to that point
                next_ik4_pos
            } else if let Some(ik3_pos) = find_next_segment_start("IK3", &contents, 0) {
                // If there's an IK3 at a segment boundary, extract up to that point
                ik3_pos
...
mod tests {
    ...
    fn test_get_loop_2110() {
        ...
        assert_eq!(
            loop2110.ctx_segments[0].ctx01_context_name,
            "SITUATIONAL TRIGGER"
        );
