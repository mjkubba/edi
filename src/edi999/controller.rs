...
mod tests {
    ...
    fn test_write_999_with_ctx_segments() {
        ...
        assert!(
            output.contains("CTX*CLM01:123456789~"),
            "Special format CTX segment not found in output"
        );
        assert!(
            output.contains("CTX*SITUATIONAL TRIGGER*CLM*43**5:3*C023:1325~"),
        ...
        assert!(
            output.contains("SE*16*"),
            "SE segment doesn't have proper values"
        );
        assert!(
            output.contains("AK9*P*3*3*1~"),
            "AK9 segment doesn't have proper values"
        );
        assert!(
            output.contains("GE*1*"),
            "GE segment doesn't have proper values"
        );
        assert!(
            output.contains("IEA*1*"),
            "IEA segment doesn't have proper values"
        );
    }
