...
mod tests {
    ...
    fn test_get_loop2010d() {
        ...
        assert_eq!(
            loop2010d.nm1_segments.id_code_qualifier,
            Some("MI".to_string())
        );
        assert_eq!(
            loop2010d.nm1_segments.id_code,
            Some("123456789B".to_string())
        );

        assert_eq!(loop2010d.ref_segments.len(), 1);
        assert_eq!(
            loop2010d.ref_segments[0].reference_id_number_qualifier,
            "SY"
        );
    ...
    fn test_write_loop2010d() {
        let loop2010d = Loop2010D {
            nm1_segments: NM1 {
                ...
                id_code: Some("123456789B".to_string()),
                member_number: None,
            },
