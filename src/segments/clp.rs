...
pub fn get_clp(bpr_content: String) -> CLP {
    ...
    if clp_parts.get(4).is_some() {
        clp05_patient_responsibility_amount = get_element(&clp_parts, 4);
    }
    if clp_parts.get(7).is_some() {
        clp08_facility_type_code = get_element(&clp_parts, 7);
    }
    if clp_parts.get(8).is_some() {
        clp09_claim_frequency_code = get_element(&clp_parts, 8);
    }
    if clp_parts.get(9).is_some() {
        clp10_patient_status_code = get_element(&clp_parts, 9);
    }
    if clp_parts.get(10).is_some() {
        clp11_diagnosis_related_group = get_element(&clp_parts, 10);
    }
    if clp_parts.get(11).is_some() {
        clp12_drg_weight = get_element(&clp_parts, 11);
    }
    if clp_parts.get(12).is_some() {
        clp13_percent_discharge_fraction = get_element(&clp_parts, 12);
    }

    CLP {
        clp01_patient_control_number: get_element(&clp_parts, 0),
        clp02_claim_status_code: get_element(&clp_parts, 1),
        clp03_total_claim_charge_amount: get_element(&clp_parts, 2),
        clp04_total_claim_payment_amount: get_element(&clp_parts, 3),
        clp05_patient_responsibility_amount,
        clp06_claim_filing_indicator_code: get_element(&clp_parts, 5),
        clp07_payer_claim_control_number: get_element(&clp_parts, 6),
