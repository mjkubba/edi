...
pub fn write_ik4(ik4: IK4) -> String {
    ...
    let position = if ik4.ik401_position_in_segment.is_empty() {
        "1"
    } else {
        &ik4.ik401_position_in_segment
    };
    let reference = if ik4.ik402_data_element_reference_number.is_empty() {
        "0"
    } else {
        &ik4.ik402_data_element_reference_number
    };
    let error_code = if ik4
        .ik403_implementation_data_element_syntax_error_code
        .is_empty()
    {
        "1"
    } else {
        &ik4.ik403_implementation_data_element_syntax_error_code
    };
