...
mod tests {
    ...
    fn test_get_msg() {
        let msg_content =
            "MSG*PLEASE CONTACT CUSTOMER SERVICE FOR ADDITIONAL INFORMATION".to_string();
    ...
    fn test_get_msg_with_control_code() {
        let msg_content =
            "MSG*PLEASE CONTACT CUSTOMER SERVICE FOR ADDITIONAL INFORMATION*NL*1".to_string();
        let msg = get_msg(msg_content);
    ...
    fn test_write_msg() {
        let msg = MSG {
            msg01_free_form_message_text:
                "PLEASE CONTACT CUSTOMER SERVICE FOR ADDITIONAL INFORMATION".to_string(),
        ...
        assert_eq!(
            msg_content,
            "MSG*PLEASE CONTACT CUSTOMER SERVICE FOR ADDITIONAL INFORMATION*NL*1~"
        );
    }
