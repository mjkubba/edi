...
pub fn get_1000bs(contents: &str) -> (Loop1000bs, String) {
    let mut contents = contents.to_string();
    let mut loop1000bs = Loop1000bs::default();

    // Parse N1 segment for payee identification
    if contents.contains("N1") {
        let n1_content = get_segment_contents("N1", &contents);
        let n1_parts: Vec<&str> = n1_content.split('*').collect();

        // Check if this is a payee identification segment (N101=RM)
        if n1_parts.len() > 0 && n1_parts[0] == "RM" {
            info!("N1 segment found for Payee Identification, ");

            loop1000bs.n1_segments = N1 {
                ...
                && check_if_segment_in_loop("REF", "ENT", &contents)
            {
                ...
                loop1000bs.ref_segments.push(ref_segment);
            ...
            while contents.contains("PER") && check_if_segment_in_loop("PER", "ENT", &contents) {
                ...
                loop1000bs.per_segments.push(per_segment);
    ...
    return (loop1000bs, contents);
}

pub fn write_loop1000b(loop1000bs: Loop1000bs) -> String {
    let mut result = String::new();

    // Write N1 segment
    if !loop1000bs.n1_segments.entity_id.is_empty() {
        result.push_str("N1*");
        result.push_str(&loop1000bs.n1_segments.entity_id);
        result.push_str("*");
        result.push_str(&loop1000bs.n1_segments.entity_name);

        if !loop1000bs.n1_segments.id_code_qualifier.is_empty() {
            result.push_str("*");
            result.push_str(&loop1000bs.n1_segments.id_code_qualifier);

            if !loop1000bs.n1_segments.id_code.is_empty() {
                result.push_str("*");
                result.push_str(&loop1000bs.n1_segments.id_code);
            }
        }
    ...
    if let Some(n3) = &loop1000bs.n3_segments {
    ...
    if let Some(n4) = &loop1000bs.n4_segments {
        if !n4.city.is_empty() {
            result.push_str("N4*");
    ...
    // Write REF segments
    for ref_segment in &loop1000bs.ref_segments {
    ...
    // Write PER segments
    for per_segment in &loop1000bs.per_segments {
