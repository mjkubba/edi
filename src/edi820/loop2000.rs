...
pub fn get_loop_2000s(mut contents: String) -> (Vec<Table2>, String) {
    ...
    while contents.contains("ENT")
        ...
        && contents.len() < original_length + 100
    {
        // Additional safety check

        safety_counter += 1;
        info!(
            "Loop2000 iteration {}, content length: {}",
            safety_counter,
            contents.len()
        );
        ...
        if new_contents == contents {
            info!(
                "Warning: No progress made in parsing Loop2100s, breaking to avoid infinite loop"
            );
...
pub fn write_loop2000(table2s: Vec<Table2>) -> String {
    ...
    for table2 in table2s {
        ...
        if !table2.ent_segments.ent02_entity_id_code.is_empty()
            || !table2
                .ent_segments
                .ent03_identification_code_qualifier
                .is_empty()
