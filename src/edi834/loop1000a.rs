...
pub fn get_loop1000a(mut contents: String) -> (Loop1000A, String) {
    let mut loop1000a = Loop1000A::default();
    ...
    if let Some(n1_start) = contents.find("N1*") {
        if let Some(n1_end) = contents[n1_start..].find("~") {
            ...
            loop1000a.n1 = get_n1(n1_content.to_string());
    ...
    if let Some(n3_start) = contents.find("N3*") {
        ...
        let next_major = ["N1*", "N4*", "PER*", "REF*", "Loop1000B", "Loop2000"]
            .iter()
        ...
        if next_major.is_none() || n3_start < next_major.unwrap() {
            if let Some(n3_end) = contents[n3_start..].find("~") {
                ...
                loop1000a.n3 = Some(get_n3(n3_content.to_string()));
    ...
    if let Some(n4_start) = contents.find("N4*") {
        ...
        let next_major = ["N1*", "PER*", "REF*", "Loop1000B", "Loop2000"]
            .iter()
            .filter_map(|seg| contents.find(seg))
            .min();
        ...
        if next_major.is_none() || n4_start < next_major.unwrap() {
            if let Some(n4_end) = contents[n4_start..].find("~") {
                ...
                loop1000a.n4 = Some(get_n4(n4_content.to_string()));
    ...
    while let Some(per_start) = contents.find("PER*") {
        ...
        let next_major = ["N1*", "REF*", "Loop1000B", "Loop2000"]
            .iter()
            .filter_map(|seg| contents.find(seg))
            .min();
        ...
        if let Some(per_end) = contents[per_start..].find("~") {
            ...
            loop1000a.per_segments.push(get_per(per_content.to_string()));
    ...
    while let Some(ref_start) = contents.find("REF*") {
        ...
        let next_major = ["N1*", "Loop1000B", "Loop2000"]
            .iter()
            .filter_map(|seg| contents.find(seg))
            .min();
        ...
        if let Some(ref_end) = contents[ref_start..].find("~") {
            ...
            loop1000a.ref_segments.push(get_ref(ref_content.to_string()));
    ...
    info!("Parsed Loop1000A: {:?}", loop1000a);
    (loop1000a, contents)
}

pub fn write_loop1000a(loop1000a: Loop1000A) -> String {
    let mut result = String::new();
    
    result.push_str(&write_n1(loop1000a.n1));
    result.push_str("\n");
    
    if let Some(n3) = loop1000a.n3 {
        result.push_str(&write_n3(n3));
        result.push_str("\n");
    }
    
    if let Some(n4) = loop1000a.n4 {
        result.push_str(&write_n4(n4));
        result.push_str("\n");
    }
    
    for per_segment in loop1000a.per_segments {
        result.push_str(&write_per(per_segment));
        result.push_str("\n");
    }
    
    for ref_segment in loop1000a.ref_segments {
