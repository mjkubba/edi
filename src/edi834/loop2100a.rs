...
pub fn get_loop2100a(mut contents: String) -> (Loop2100A, String) {
    ...
    while let Some(per_start) = contents.find("PER*") {
        ...
        let next_major = ["N3*", "N4*", "DMG*", "REF*", "NM1*", "HD*", "COB*", "DSB*", "INS*", "SE*"]
            .iter()
        ...
        if let Some(per_end) = contents[per_start..].find("~") {
            ...
            loop2100a.per_segments.push(get_per(per_content.to_string()));
    ...
    if let Some(n3_start) = contents.find("N3*") {
        ...
        let next_major = ["N4*", "DMG*", "REF*", "NM1*", "HD*", "COB*", "DSB*", "INS*", "SE*"]
            .iter()
            .filter_map(|seg| contents.find(seg))
            .min();
    ...
    while let Some(ref_start) = contents.find("REF*") {
        ...
        if let Some(ref_end) = contents[ref_start..].find("~") {
            ...
            loop2100a.ref_segments.push(get_ref(ref_content.to_string()));
